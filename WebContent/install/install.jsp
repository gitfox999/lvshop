		...
		if (databaseType.equalsIgnoreCase("mysql")) {
			...
			if (status.equals("success")) {
				if (StringUtils.equals(isCreateDatabase, "true")) {
					try {
						connection = DriverManager.getConnection("jdbc:mysql://" + databaseHost + ":" + databasePort + "/" + databaseName + "?useUnicode=true&characterEncoding=" + DATABASE_ENCODING, databaseUsername, databasePassword);
						...
						try {
							...
							try {
								...
								connection = DriverManager.getConnection("jdbc:mysql://" + databaseHost + ":" + databasePort + "/" + databaseName + "?useUnicode=true", databaseUsername, databasePassword);
							} catch (SQLException e2) {
								try {
									exception = stackToString(e2);
									connection = DriverManager.getConnection("jdbc:mysql://" + databaseHost + ":" + databasePort + "/test?useUnicode=true", databaseUsername, databasePassword);
								} catch (SQLException e3) {
									exception = stackToString(e3);
								}
							}
						}
					}
